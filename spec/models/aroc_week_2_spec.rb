...
# ----------------------
  ...
  it "13. sort an order's items by name" do
    expected_result = [@item_4, @item_2, @item_5, @item_3]

    # ----------------------- Using Ruby -------------------------
    #order = Order.find(@order_3.id)
    #sorted_items = order.items.sort_by { |item| item.name }
    # ------------------------------------------------------------

    # ------------------ Using ActiveRecord ----------------------
    sorted_items = Order.joins(:items).find(@order_3.id).items.order(:name)
    # ------------------------------------------------------------

    # Expectation
    expect(sorted_items).to eq(expected_result)
  end
